...
export function App() {
  ...
  const [users, setUsers] = useState<User[]>(() => {
    try {
      const isUsersCleaned = localStorage.getItem('dslng_user_v10_admin_pass');
      if (!isUsersCleaned) {
        localStorage.setItem('dslng_users', JSON.stringify(INITIAL_USERS));
        localStorage.removeItem('dslng_current_user');
        localStorage.setItem('dslng_user_v10_admin_pass', 'true');
  ...
  const [tickets, setTickets] = useState<Ticket[]>(() => {
    try {
      const isCleared = localStorage.getItem('dslng_clean_tickets_v12_empty');
      if (!isCleared) {
        localStorage.removeItem('dslng_tickets');
        localStorage.setItem('dslng_clean_tickets_v12_empty', 'true');
        return [];
      }
      ...
      prev.map((t) =>
        t.id === ticketId
          ? {
              ...t,
              status,
              resolution_notes: notes !== undefined ? notes : t.resolution_notes,
              assigned_to: assignment?.assigned_to !== undefined ? assignment.assigned_to : t.assigned_to,
              assigned_to_name: assignment?.assigned_to_name !== undefined ? assignment.assigned_to_name : t.assigned_to_name,
              assigned_to_id: assignment?.assigned_to_id !== undefined ? assignment.assigned_to_id : t.assigned_to_id,
              picked_up_at: assignment?.picked_up_at !== undefined ? assignment.picked_up_at : t.picked_up_at,
