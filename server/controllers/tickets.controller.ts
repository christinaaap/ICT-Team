...
export const createTicket = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const newTicket: Ticket = {
      ...
      status: initialStatus || (assigned_to ? 'In Progress' : 'Open'),
      assigned_to: assigned_to || undefined,
      assigned_to_name: assigned_to_name || assigned_to || undefined,
      assigned_to_id: assigned_to_id || undefined,
      picked_up_at: picked_up_at || (assigned_to ? new Date().toISOString() : undefined),
...
export const updateTicketStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { status, notes, assigned_to, assigned_to_name, assigned_to_id, picked_up_at } = req.body;

    const assignment = (assigned_to !== undefined || assigned_to_name !== undefined || assigned_to_id !== undefined || picked_up_at !== undefined)
      ? { assigned_to, assigned_to_name, assigned_to_id, picked_up_at }
      : undefined;

    const updated = db.updateTicketStatus(ticketId, status, notes, assignment);
    if (!updated) {
